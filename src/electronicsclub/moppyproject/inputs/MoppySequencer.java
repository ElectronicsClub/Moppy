...
import electronicsclub.moppyproject.MoppyStatusConsumer;
...
public class MoppySequencer implements MetaEventListener, Transmitter {
    ...
    ArrayList<MoppyStatusConsumer> listeners = new ArrayList<MoppyStatusConsumer>(
            1);
    ...
    public void loadFile(String filePath) throws InvalidMidiDataException,
            IOException, MidiUnavailableException {
        sequencer.stop();
        Sequence sequence = MidiSystem.getSequence(new File(filePath));

        sequencer.setSequence(sequence);
        System.out.println("Loaded sequence with "
                + (sequence.getTracks().length - 1) + " MIDI channels.");
    ...
    public void setReceiver(Receiver receiver) {
        try {
            ...
            Logger.getLogger(MoppySequencer.class.getName()).log(Level.SEVERE,
                    null, ex);
        }
    }
