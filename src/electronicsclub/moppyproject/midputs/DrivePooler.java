package electronicsclub.moppyproject.midputs;
...
import electronicsclub.moppyproject.Constants;
...
public class DrivePooler implements Receiver, Transmitter {
    ...
    public void send(MidiMessage message, long timeStamp) {
        if (receiver != null)
            if (message instanceof ShortMessage
                    && (message.getMessage()[0] & 0xFF) > 127
                    && (message.getMessage()[1] & 0xFF) < 160)
    ...
    private MidiMessage roundRobinMap(MidiMessage message) {
        ...
        try {
            if (mappedMessage.getChannel() >= startInput
                    && mappedMessage.getChannel() <= endInput) {
                if (mappedMessage.getCommand() == ShortMessage.NOTE_OFF
                        || mappedMessage.getCommand() == ShortMessage.NOTE_ON
                        && mappedMessage.getData2() == 0) {
                    ...
                    for (int n = startOutput; n <= endOutput; n++)
                        if (currentNotes[n] == noteNumber) {
                            ...
                            mappedMessage.setMessage(
                                    mappedMessage.getCommand(), n,
                                    mappedMessage.getData1(),
                                    mappedMessage.getData2());
                        }
                    ...
                    mappedMessage.setMessage(mappedMessage.getCommand(),
                            targetNote, mappedMessage.getData1(),
                            mappedMessage.getData2());

                    rrNextNote++;
                }
            } else
                return message; // It's not in the mapping range, let it pass
                                // through...
        } catch (InvalidMidiDataException ex) {
            Logger.getLogger(DrivePooler.class.getName()).log(Level.SEVERE,
                    null, ex);
        }
        ...
        try {
            ...
            if (mappedMessage.getChannel() >= startInput
                ...
                if (mappedMessage.getCommand() == ShortMessage.NOTE_OFF
                    ...
                    mappedMessage.setMessage(mappedMessage.getCommand(),
                            targetChannel, mappedMessage.getData1(),
                            mappedMessage.getData2());
                }
            } else
                return message; // It's not in the mapping range, let it pass
