...
public class MoppyCOMBridge {
    ...
    public MoppyCOMBridge(String portName) throws NoSuchPortException,
            PortInUseException, UnsupportedCommOperationException, IOException {
        CommPortIdentifier cpi = CommPortIdentifier.getPortIdentifier(portName);
        com = (SerialPort) cpi.open("MoppyDesk", 2000);
        com.setSerialPortParams(SERIAL_RATE, SerialPort.DATABITS_8,
     ...
     * 
     * @param pin
     *            Controller pin to handle ntoe
     * @param periodData
     *            length of period in microSeconds
     */
    public void sendEvent(byte pin, int periodData) {
        sendEvent(pin, (byte) (periodData >> 8 & 0xFF),
                (byte) (periodData & 0xFF));
    }
    ...
    private void sendArray(byte[] message) {
        try {
            ...
            Logger.getLogger(MoppyCOMBridge.class.getName()).log(Level.SEVERE,
                    null, ex);
        }
    }
    ...
    public void resetDrives() {
        if (isOutputOpen) {
            ...
            try {
                ...
                Logger.getLogger(MoppyCOMBridge.class.getName()).log(
    ...
    public void close() {
        if (os != null) {
            if (isOutputOpen) { // Only attempt to silence if the output is
                                // still open.
                silenceDrives();
