...
import electronicsclub.moppyproject.OutputSetting;
import electronicsclub.moppyproject.outputs.MoppyCOMBridge;
import electronicsclub.moppyproject.outputs.MoppyMIDIOutput;
import electronicsclub.moppyproject.outputs.MoppyPlayerOutput;
import electronicsclub.moppyproject.outputs.MoppyReceiver;
...
public class MoppyControlWindow extends javax.swing.JFrame {
    ...
    private void loadOutputSettings() {
        OutputSetting[] os = (OutputSetting[]) app
                .getPreferenceObject(Constants.PREF_OUTPUT_SETTINGS);
        if (os == null) {
            for (int i = 1; i <= 16; i++)
                outputSettings[i - 1] = new OutputSetting(i);
            app.putPreferenceObject(Constants.PREF_OUTPUT_SETTINGS,
                    outputSettings);
        } else
            outputSettings = os;
    }
    ...
    private void initComponents() {
        ...
        inputSelectBox.setModel(new javax.swing.DefaultComboBoxModel(
                new String[] { "MIDI File", "MIDI IN Port" }));
        ...
        mainOutputPanel.setBorder(javax.swing.BorderFactory
                .createEtchedBorder());
        mainOutputPanel.setPreferredSize(new java.awt.Dimension(350, 400));
        mainOutputPanel.setLayout(new javax.swing.BoxLayout(mainOutputPanel,
                javax.swing.BoxLayout.Y_AXIS));

        connectButton.setText("Connect");
        connectButton
                .setToolTipText("Saves current output settings and connects as specified.");
    ...
    private void connect() {
        try {
            ...
            // If pooling is enabled, send messages through pooler, otherwise
            // bypass it
            inputSelectBox.setEnabled(false);
            if (poolingControls1.isPoolingEnabled()) {
                currentInputPanel.getTransmitter().setReceiver(
                        poolingControls1.getDrivePooler());
            ...
            Logger.getLogger(MoppyControlWindow.class.getName()).log(
    ...
    private void initializeReceivers() throws NoSuchPortException,
        ...
        for (int ch = 1; ch <= 16; ch++) {
            OutputSetting os = outputSettings[ch - 1]; // OutputSettings are
                                                       // 0-indexed
            if (os.enabled)
                ...
                if (os.type.equals(OutputSetting.OutputType.MOPPY)) {
                    if (!outputPlayers.containsKey(os.comPort))
                        outputPlayers.put(os.comPort, new MoppyPlayerOutput(
                                new MoppyCOMBridge(os.comPort)));
                ...
                // MIDIPlayer/Receivers are grouped by MIDI output name
                    ...
                    if (!outputPlayers.containsKey(os.midiDeviceName))
                        outputPlayers.put(os.midiDeviceName,
                                new MoppyMIDIOutput(os.midiDeviceName));
    ...
    private void updateInputPanel() {
        ...
        if (inputSelectBox.getSelectedIndex() == 0)
    ...
    private electronicsclub.moppyproject.ui.PoolingControls poolingControls1;
